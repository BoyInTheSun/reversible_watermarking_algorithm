...
# 找最大最小点 img是传入图片 img1是比对图片 并嵌入数据
def finding_max_and_min(img, img1, input_image_path):
    width = len(img)
    height = len(img[0])
    frequency = list()
    ...
    min1 = maxp - 1
    min2 = maxp + 1
    while(frequency[min1] != 0 and frequency[min2] !=0):
        if(frequency[min1] == 0):
            min = min1
